        ...
        if parser.unknown is False:
            ...
            for i in expertise:
                ...
                if len(j) != 1:
                    for k in j[1:]:
                        l = k.split(',')
                        expertise_vocabulary.extend(l)
                else:
                    for k in j:
                        l = k.split(',')
                        expertise_vocabulary.extend(l)
            expertise_vocabulary = list(filter(None, expertise_vocabulary))
            expertise_vocabulary = list(OrderedDict.fromkeys(expertise_vocabulary))
            print(expertise_vocabulary)
            experience_vocabulary = []
            for i in experience:
                experience_vocabulary.append(i.encode('ascii', 'ignore').decode("utf-8"))
            ...
            tfidf = TfidfVectorizer(vocabulary=expertise_vocabulary, stop_words='english', ngram_range=(1, 2))
